...
function FeatureCard({ href, icon, itemImage, title, description, color }: { href: string, icon: React.ReactNode, itemImage: string, title: string, description: string, color: string }) {
  ...
  return (
    <Link href={href} className={`group block relative p-6 md:p-8 bg-card rounded-3xl border border-border transition-all hover:-translate-y-2 hover:shadow-2xl overflow-hidden ${bgClass.split(' ')[0]} ${bgClass.split(' ')[1]}`}>
